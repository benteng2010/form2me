...
if($stype&&$dbType=='sqlite')
{
	$idlist=reSetRs($idlist);
}
	...
	$sql = "SELECT m.id,m.cateid,m.subject,m.postdate,m.hits,m.ext_docket,c.catename ".$condition." ORDER BY istop DESC,isvouch DESC,isbest DESC,orderdate DESC,postdate DESC,";
	$dbType=='sqlite'?$sql.=" m.id ":$sql.=" id ";
	$sql.=" DESC LIMIT ".$offset.",".$psize;
	$searchlist = $DB->qgGetAll($sql);
if($dbType=='sqlite')
{
	$searchlist=reSetRs($searchlist);
}
	if(!$searchlist)
	{
		qgheader();
